...
function Sideabar({user,onLogout, isClose, setClose}) {
    const { pathname } = useLocation();
    const navigate = useNavigate();
    const sidebarRef = useRef();

    const[mobileOpen, setMobileOpen] = useState(false); //when sidebar is open it is true
    const[activeHover, setActiveHover] = useState(null);

    const {name: username = "User", email = "user@gmail.com"} = user || {};
    const initial = username.charAt(0).toUpperCase();

    //top check for overflow in mobile:(Prevent body scrolling when sidebar is open in mobiles)
    useEffect(() => {
  ...
  // click outside the box sidebar collapsed
  useEffect(() => {
    ...
    document.addEventListener("mousedown", handleClickOutside); // Whenever the mouse is pressed anywhere...
  ...
  // a small component: (it returns <li>...</li>)
      ...
      <motion.li key={text} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
        <Link
          ...
          className={cn(
            ...
            isClose? sidebarStyles.menuItem.collapsed : sidebarStyles.menuItem.expanded
          )}
        ...
        >
          ...
          {activeHover === text && !isActive && !isClose && (
    ...
    >
        <div className={sidebarStyles.sidebarInner.base}>
              <button onClick={toggleSidebar} className={sidebarStyles.toggleButton.base}>
                 <motion.div
              initial={{ rotate: 0 }}
              animate={{ rotate: isClose ? 0 : 180 }}
              transition={{ duration: 0.3 }}
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <polyline points={isClose? "9 18 15 12 9 6" : "15 18 9 12 15 6"}></polyline>
              </svg>
            </motion.div>
              </button>
              ...
              <div className={cn(
                ...
                isClose ? sidebarStyles.userProfileContainer.collapsed : sidebarStyles.userProfileContainer.expanded
              )}>
              ...
              <div className={cn(
                ...
                isClose? sidebarStyles.footerContainer.collapsed:sidebarStyles.footerContainer.expanded
              )}>
                <Link className={cn(
                  sidebarStyles.footerLink.base,
                  isClose && sidebarStyles.footerLink.collapsed,
                )} to={"https://github.com"}>

                  <HelpCircle size={20} className="text-gray-500" />
                  {!isClose && <span>Support</span>}
                  </Link>

                  <button onClick={handleLogout} className={cn(
                    sidebarStyles.logoutButton.base,
                    isClose && sidebarStyles.logoutButton.collapsed
                  )}>
                   ...
                   <div className={sidebarStyles.mobileHeader}>
                        <div className={sidebarStyles.mobileUserContainer}>
                              ...
                              <div>
                                ...
                                <p className='text-se=m text-gray-500'>{email}</p>
                              </div>
                        </div>
