...
const Profile = ({onUpdateProfile, onLogout}) => {
  ...
  //handle api request
  const handleApiRequest = useCallback(async(method, url, data = null ) => {
      try {
        setLoading(true);
      const config = {
        method,
  ...
  useEffect(() => {
      const fetchUserData = async () => {
         try {
            const data = await handleApiRequest("get","/users/current-user")
            if(data){
            ...
            toast.error("failed to fetch user data") //we use toast instead of alert() as it doesnot interrupt the user and automaticcaly get disappered
         }
      };
      fetchUserData()
  },[handleApiRequest])
 ...
 const handleInputChange = useCallback((e) => {
  ...
  //saveprofile
   const handleSaveProfile = async() => {
    try {
       const data = await handleApiRequest("patch","/user/update-profile", tempUser) 
       if(data){
  ...
  //to change passwrod
   const handlePasswordSubmit = async(e) => {
      ...
      try {
        await handleApiRequest("patch", "/user/password", { 
        ...
        <div className={profileStyles.content}>
          <div className={profileStyles.grid}>
            <div className={profileStyles.card}>
              ...
              <div className={profileStyles.card}>
                  <h2 className={profileStyles.card}>
